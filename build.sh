...
for DIR in $ROOT_DIRS; do
  mkdir $TMP_DIR/$DIR
  FILES="`find $DIR -path '*.svn*' -prune -or -path '*.idl' -prune -or -type f -print | grep -v \~`"
  echo $FILES >> files
  tar c $FILES | (cd $TMP_DIR/; tar xvf - )
done
...
perl -pi -e "s/.*updateURL.*//" install.rdf
perl -pi -e "s/.*updateKey.*//" install.rdf
zip -r ../$APP_NAME.mozilla.xpi *
